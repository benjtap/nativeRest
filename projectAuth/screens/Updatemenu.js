...
const UpdateMenu = (props) => {
  ...
  useEffect(() => {
    ...
    if(!didFetch){
      ...
      fetchMenuDataForPosts(oparamname);
      setDidFetch(true)
  ...
  const validateForm = () => {
    ...
    if (!selectedText)  { 
      ...
      if(csvData.length===0 )
         {
          errors.csvData = 'one menu  is required.'; 
          setIsFormValid(false);
          return;
         } 
         else
         {
          if ((selectedText) && (csvData.length!=0))  {
            setErrors(errors); 
           setIsFormValid(true); 
          }
  ...
  const onNextStep1= () => {
    ...
    const fetchMenuDataForPosts = async (oparamname) => {
      const limit = 30;
      ...
      let url = `/Webhttp/getMenu`
  
      await axiosInstance.post(url,getMenuPost)
  
        .then(({ data }) => {
       
         
      
          setData(JSON.parse(data.jsonarray) )
  
         setSelectedText(data.filename)
        })
    ...
    const exportnewCsv = async () => {
        ...
        { 
         ...
         const url =`/Webhttp/updatemenu`;
  
         await axiosInstance.post(url,createmenuPost) .then((res) => {
          navigation.navigate('MENUSAPP')
        })
       ...
       const GetItemInfo = ({item,index,level}) => {
        ...
        if(level==1)
         for(var p in data){
           ...
           for(var p in data){
             ...
             var rpq = pindex.items[pq]
               if((rpq.title !=undefined) && (rpq.items[index]==item))
                {
                  title = rpq.title
                 // console.log(item)
         ...
         for(var p in data){
           ...
           for(var lpq in rpq.items){
             ...
             if((gpq.title !=undefined)  && (gpq.items[index]==item)){
               title = gpq.title
        ...
        const updateChild = (item,index,level) => {
       
          ClearModal()
          setStep1Data({ menuname: item.title  }) 
          setStep2Data({ audioname:  item.audioname})
         ...
         setisItem(item);
          ...
          setTitle(() => {
            let myarrtitle= []
            console.log(item)
            for(var pindex in item.items){
              var qval = item.items[pindex]
             
              myarrtitle.splice(i, 0, qval.makchim);
              i=i+1;
             }
          return  myarrtitle;
    ...
    {data.map((item, index) => (
        <View key={index} style={{borderBottomColor: 'black', borderBottomWidth: .5,marginTop:25}}>
          ...
          <View style={{ flexDirection:"row" }}>
             ...
             onPress={() => updateChild(item,index,0)} /> 
             </View> 
             <View style={{ marginHorizontal: 10,marginTop: 5 }}>
          ...
          />    מקש {item.makchim} </Text>
                ...
                <View style={{ marginHorizontal: 10,marginTop: 5 }}>
                 <Button   style={styles.button1}  title="ערוך"  
                onPress={() => updateChild(item,index,1)} /> 
                </View> 
                <View style={{ marginHorizontal: 10,marginTop: 5 }}>
                <Button   style={styles.button1}  title="נגן"  
                onPress={() => listenAudiorecord(item.audioname)} /> 
       ...
       <View style = {{flex:1,flexDirection:'row',flexWrap:'wrap',justifyContent:'center'}}>
         {item.items !=null && 
          ...
          />   מקש {item.makchim}  </Text>
             ...
             <View style={{ marginHorizontal: 10,marginTop: 5 }}>
              <Button   style={styles.button1}  title="ערוך"  
             onPress={() => updateChild(item,index,2)} /> 
             </View> 
             <View style={{ marginHorizontal: 10,marginTop: 5 }}>
             <Button   style={styles.button1}  title="נגן"  
             onPress={() => listenAudiorecord(item.audioname)} /> 
...
<View style={{ flexDirection:"row" }}>
     ...
     <View style={{ marginHorizontal: 10,marginTop: 5 }}>
            <Button title="בטל" onPress={() =>   navigation.navigate('MENUSAPP')} style={{ padding: 20, }} />
     </View>
